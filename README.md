...
- **Bilingual, split URLs (SEO):** English at `/`, 简体中文 under `/zh/`, with
  ...
  generated from single-source pages in `src/` by `scripts/build_i18n.py`
...
├── scripts/
│   ├── build_i18n.py    # src/*.html → en + zh outputs + sitemap.xml (stdlib only)
...
├── sitemap.xml     # GENERATED by build_i18n.py (loc + lastmod)
├── README.md       # This file
...
2. Regenerate the outputs and sitemap: `python3 scripts/build_i18n.py`
...
`/zh/`，互挂 hreflang），由 `scripts/build_i18n.py` 从 `src/` 单源生成——改内容
